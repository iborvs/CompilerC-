...
extern vector<string> struc_symbol;
...
int evaluation()
{
    if(type()) {
        ...
        if(id()) {
            ...
            else if(words[token_i].value == ".") {   //结构体
                if(struc_symbol.empty() || struc_symbol[struc_symbol.size() - 1] != words[token_i-1].value) {
                    cout<<"无此结构体"<<endl;
                    return 0;
                }
                else {
                    token_i++;
                    if(id()) {
                        string tmp_name = words[token_i-1].value;
                        int flag = 0;
                        for(int i = 0; i < struc_symbol.size() - 1; i++) {
                            if(words[token_i-1].value == struc_symbol[i]) {
                                flag = 1;
                                break;
                            }
                        }
                            ...
                            if(words[token_i].value == "=") {
                                ...
                                if(r_value()) {
                                    ...
                                    token_i++;
                                        //四元式
                                        qua.s[0] = "=";
                                        qua.s[3] = struc_symbol[struc_symbol.size() - 1];
                                        qua.s[2] = tmp_name;
                                        out_qua();
                                        return 1;
                                    }
